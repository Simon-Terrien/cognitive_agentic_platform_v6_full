...
def test_chat_requires_auth_when_enabled(monkeypatch, tmp_path):
    ...
    async def run():
        transport = httpx.ASGITransport(app=app)
        async with httpx.AsyncClient(transport=transport, base_url='http://testserver') as client:
            unauthorized = await client.post('/api/chat', json={'message': 'say ok', 'model_id': 'mock_static'})
...
def test_metrics_and_request_id_headers(monkeypatch, tmp_path):
    ...
    async def run():
        ...
        async with httpx.AsyncClient(transport=transport, base_url='http://testserver') as client:
            ...
            await client.post('/api/auth/login', json={'email': 'operator@test.local', 'password': 'wrong'})
