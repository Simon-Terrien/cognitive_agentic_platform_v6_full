...
class AgentEngine:
    ...
    def _memory_context(self, session_id: str, query: str) -> tuple[list[str], list[dict]]:
        ...
        metrics_registry.record_agent_memory_hits(len(retrieved))
    ...
    def _build_traces(self, query: str, spec, provider, session_id: str | None = None) -> tuple[dict, list[dict], list[str]]:
        settings = get_settings()
        state = CognitiveState(goal=query)
        plan = self.planner.create_plan(query)
        usage = ExecutionUsage()
        resolved_session_id = session_id or spec.id
        memory_context, memory_traces = self._memory_context(session_id=resolved_session_id, query=query)
        traces = [
        ...
        if usage.recursion_depth > settings.agent_max_recursion_depth:
            ...
            metrics_registry.record_agent_governor_event('max_recursion_depth_exceeded')
        ...
        if len(limited_steps) < len(plan.steps):
            ...
            metrics_registry.record_agent_governor_event('max_iterations_reached')
        ...
        for idx, step in enumerate(limited_steps, start=1):
            ...
            if usage.tool_calls >= settings.agent_max_tool_calls:
                ...
                metrics_registry.record_agent_governor_event('max_tool_calls_reached')
                break
            ...
            metrics_registry.record_agent_tool_policy('allow' if tool_decision.allowed else 'deny')
            ...
            if prompt_tokens > settings.agent_max_prompt_tokens:
                ...
                metrics_registry.record_agent_governor_event('max_prompt_tokens_reached')
                break
            if usage.total_tokens + prompt_tokens > settings.agent_max_total_tokens:
                ...
                metrics_registry.record_agent_governor_event('max_total_tokens_reached')
                break
            ...
            state.remember(note)
            self.memory.append(session_id=resolved_session_id, goal=query, note=note, importance=min(1.0, 0.35 + 0.1 * idx))
            notes.append(note)
        ...
        if final_prompt_tokens <= settings.agent_max_prompt_tokens and usage.total_tokens + final_prompt_tokens <= settings.agent_max_total_tokens:
            ...
            metrics_registry.record_agent_governor_event('final_answer_budget_exceeded')
    ...
    def run(self, query: str, model_id: str | None = None, session_id: str | None = None) -> dict:
        ...
        payload, traces, _ = self._build_traces(query, resolution.resolved_model, provider, session_id=session_id)
    ...
    def run_stream(self, query: str, model_id: str | None = None, session_id: str | None = None) -> Generator[dict, None, None]:
        resolution = self.router.resolve(self.providers, model_id, query=query, requires_stream=True)
        provider = self.providers.get(resolution.resolved_model)
        settings = get_settings()
        usage = ExecutionUsage()
        resolved_session_id = session_id or resolution.resolved_model.id
        if resolution.fallback_reason:
            yield {
                'kind': 'fallback',
                'message': 'model fallback applied',
